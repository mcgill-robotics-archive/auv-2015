...
class ManeuverTask(task.Task):
	...
	def load_action_stack(self):
		if (self.phase > 0):
			my_x_axis_movment_action = x_axis_movement_action.XAxisMovementAction(self.my_autonomy, 3)
			my_position = get_transform_action.GetTransformAction(self.my_autonomy, new_target_frame)
			my_gate_colour = set_cv_target_action.SetCVTargetAction(self.my_autonomy, new_cv_target)
			self.action_stack.insert(0, my_gate_colour)
