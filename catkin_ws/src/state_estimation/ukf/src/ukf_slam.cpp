...
MatrixXd ukf_slam::observe(int objectID, double base_yaw, Transformation transform, const MatrixXd sigmas) {
  //Hide all the bits we don't care about and convert to the sensor frame
  MatrixXd conv_sigmas = transform * sigmas.block(3*index, 0, 3, sigmas.cols());
...
void ukf_slam::update(int objectID, Transformation transform, const Vector3d msmt, constVector3d covar)
{
	...
	estimator.correct(Vector3d(msmt(0),M_PI,msmt(2)), boost::bind(&ukf_slam::observe, objectId, msmt(1), _1),
	    covar.asDiagonal());
